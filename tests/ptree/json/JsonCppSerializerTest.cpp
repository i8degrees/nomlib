...
TEST_F( JsonCppSerializerTest, ExtraIO )
{
  ...
  // ASSERT_TRUE( fp->load( RESOURCE_JSON_AUCTIONS, os, nom::ISerializer::Features::Compact ) );
  // EXPECT_EQ( 4, os.size() );

  ASSERT_TRUE( fp->load( RESOURCE_JSON_INVENTORY, os, nom::ISerializer::Features::Compact ) );
  EXPECT_EQ( 2, os.size() );
